...
- **Resume page** (`/resume`): single hc- card-based layout (no layout toggle, no hero) — Professional Summary → Engineering Expertise (capability cards, reused verbatim from Home, incl. its animated stats bar right below) → Key Achievements (`rs-card` mini-cards, each with a tinted `rs-card__icon` circle instead of a badge/tech row, and equal-length descriptions for consistent card height) → Professional Experience → Relevant Experience → Education (`rs-edu-item` rows) → Certifications (`rs-cert` link grid) → Other Skills & Tools (`rs-skill-card` grid, carries the `#skills` anchor targeted by nav/footer "Skills" links) → `hc-cta` band. Professional Experience adapts Portfolio's `.hc-work` Challenge/Solution/Impact breakdown into a compact `rs-work-card` shown 3-per-row in a `rs-work-grid` (per-job items each get `mark`/`metric`/`challenge`/`solution`/`impact` fields — the latter three kept to single short sentences, since detail lives on the linked case study rather than the card — a `rs-work-card__tile` at the top of the card reusing `.hc-work__tile`'s dotted-pattern/alternating-blue-violet look at a smaller scale (mark circle + name + metric, no separate name heading below it), and an optional `note` for confidential clients, plus a decorative "Case Study" button reused from Portfolio, hidden via `caseStudy: false` on items that aren't real shippable projects e.g. Team Leadership) — it still reuses the shared `.hc-work__breakdown`/`.hc-work__point`/`.hc-work__actions` classes from `hc-shared.css` for the dt/dd labels and button row, just not the two-column `.hc-work` card shell or its sticky-stack behavior. `rs-work-card` also gets its own hover lift (`translateY(-7px)` + shadow, matching `.rs-card`'s hover elsewhere on the page). Each job is an `hc-card rs-job-group` — the same white-card treatment as `.rs-job`/Relevant Experience, just with a `rs-work-grid` of tinted `rs-card--nested`-style cards inside instead of a `rs-card-grid--nested` of mini-cards — wrapping a `rs-job__header`-styled header (`border-bottom`, no horizontal padding of its own since the outer card's own padding already insets it to match the grid cards below) and the grid, which drops to 2 columns under 1100px and 1 column under 750px. Relevant Experience keeps the older pattern — `.rs-job` wrapping a `rs-card-grid--nested` grid of `rs-card--nested` mini-cards (badge/metric row + tech tags) — since ITU's smaller, less narrative entries don't need the Challenge/Solution/Impact breakdown. `@media print` hides `.hc-nav`/`.hc-footer`/anything marked `.rs-noprint` (Engineering Expertise, stats bar, Other Skills & Tools, CTA band), flattens card chrome (shadows/backgrounds), and collapses `rs-work-grid` to a single column with `.hc-work__actions` hidden so Professional Experience prints as text only; a `.rs-print-header` (name + contact line) is hidden on screen and shown only when printing — it's the only place the name/title appears, since there's no on-screen hero.
