...
- **Resume page** (`/resume`): single hc- card-based layout (no layout toggle, no hero) — Professional Summary → Engineering Expertise (capability cards, reused verbatim from Home, incl. its animated stats bar right below) → Key Achievements (`rs-card` mini-cards, each with a tinted `rs-card__icon` circle instead of a badge/tech row, and equal-length descriptions for consistent card height) → Professional Experience → Relevant Experience → Education (`rs-edu-item` rows) → Certifications (`rs-cert` link grid) → Other Skills & Tools (`rs-skill-card` grid, carries the `#skills` anchor targeted by nav/footer "Skills" links) → `hc-cta` band. Professional Experience reuses Portfolio's `.hc-work` Challenge/Solution/Impact card verbatim (per-job items each get `mark`/`metric`/`challenge`/`solution`/`impact` fields, an alternating blue/violet tile, and an optional `note` for confidential clients, rendered via `renderTech`-free markup with a decorative "Case Study" button matching Portfolio's) — each job is a plain `rs-job-group` (a `rs-job__header`-styled header with no card wrapper, `border-bottom` instead) followed by a stack of sticky `.hc-work` cards, not a single card of mini-cards. Relevant Experience keeps the older pattern — a single `rs-job` card wrapping a `rs-card-grid--nested` grid of `rs-card--nested` mini-cards (badge/metric row + tech tags) — since ITU's smaller, less narrative entries don't need the two-column breakdown. `@media print` hides `.hc-nav`/`.hc-footer`/anything marked `.rs-noprint` (Engineering Expertise, stats bar, Other Skills & Tools, CTA band), flattens card chrome (shadows/backgrounds), and additionally collapses `.hc-work` to a single static column with the tile/actions hidden so Professional Experience prints as text only; a `.rs-print-header` (name + contact line) is hidden on screen and shown only when printing — it's the only place the name/title appears, since there's no on-screen hero.
