...
- **Work case study** (`/work/<slug>`, one per project — see Architecture): HcNav → hero (name, clamped 1-3 line description, confidentiality note if any, plus a mark tile or — if the project sets `logo` — its real app/brand logo) → Built With (tech stack chips) → Project Overview (narrative, 1-3 paragraphs) → full Challenge/Solution/Impact breakdown → prev/next project nav (tinted mark badge + category) → CTA band → HcFooter. No breadcrumb (removed), no separate metric/action-buttons row in the hero (removed as redundant with the tile and the page's own nav).
