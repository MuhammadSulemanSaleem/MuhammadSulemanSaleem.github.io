...
- **HcNav / HcFooter** (`src/components/HcNav`, `src/components/HcFooter`): shared Astro components used by all four hc- pages so the floating-pill nav (scroll-hide-on-down script, active-link highlighting via `Astro.url.pathname`, links ordered Home → Work → About) and the black card-style footer aren't duplicated per page. Certifications uses the separate `Navbar`/`Footer` components instead — the two pairs are not interchangeable (different markup, classes, and token set).
...
- Home, Work, Resume, and Contact share `HcNav` + `HcFooter` (hc- design system), with `.hc-main { padding-top: var(--navbar-height) }`. Certifications shares the legacy `Navbar` + `Footer` instead, with `padding-top: 65px` on `<main>` for the fixed nav (see Architecture)
