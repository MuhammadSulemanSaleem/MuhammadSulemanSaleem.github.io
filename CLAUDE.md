...
src/
  ...
  data/
    projects.js           # Canonical 6-category/20-project list (slug, mark, name, metric, challenge,
                           # solution, impact, note?, stack) — source for both /work's category sections
                           # and every /work/<slug> case study page
...
- **Work case study** (`/work/<slug>`, one per project — see Architecture): breadcrumb → hero (mark tile, name, metric, confidentiality note if any) → full Challenge/Solution/Impact breakdown → tech stack chips → prev/next project nav → CTA band → HcFooter
