    ...
    items: [
      {
        ...
        challenge:
          'United Global Van Line (UGVL) needed its entire lead-to-deposit funnel to run with zero tolerance for slow response or dropped leads, but voice, SMS, and voicemail automation were split across three disconnected outside vendors, and stalled leads or underperforming agents could go unnoticed for a week or more.',
        problemSolved:
        ...
        stack: ['GoHighLevel (GHL)', 'Aloware', 'Aloware AloAi', 'Retell AI', 'n8n', 'Clover', 'OpenAI', 'Inventory Link API'],
        faqs: [
          {
            q: 'What is speed-to-contact automation in moving-company sales?',
            a: "It contacts a new lead within seconds of intake through an automated voice bot call, SMS, and ringless voicemail before routing it to a live human agent. Here, an Aloware AloAi voice bot call fires at T+30 seconds and the lead escalates into the human power-dialer queue by T+10 minutes if it hasn't converted.",
          },
          {
            q: 'How does n8n tie Go High Level and Aloware together?',
            a: "n8n sits as middleware between the two, listening for CRM triggers — a new contact, a pipeline stage change, a tag update — and calling Aloware's API to start voice-bot calls, add leads to power-dialer ring groups, or sync call outcomes back into GHL as tags and stage changes, without a native integration between the platforms.",
          },
          {
            q: 'What is lead-temperature scoring and how does it affect routing?',
            a: 'Every incoming lead is scored across move-date urgency, home size, move distance, and post-contact engagement into HOT (75+), WARM (50–74), or LOW (below 50). That score determines which ring group — and which tier of closer, Senior, Junior, or Fresh — the lead is routed to, so high-value leads reach the most experienced agents first.',
          },
          {
            q: 'Why replace Retell AI, Closebot, and Slybroadcast with Aloware?',
            a: "The original build split outbound calls, inbound SMS, and voicemail drops across three separate vendors, each with its own credentials, webhooks, and conversation thread that GHL and agents couldn't fully see. Consolidating onto Aloware — already the client's human-dialer platform — cut three subscriptions down to one and put every voice call, text, and voicemail for a lead into a single unified thread.",
          },
          {
            q: 'How is AI used to score sales calls?',
            a: 'Every recorded human sales call is transcribed and scored by OpenAI against a rubric covering speed and confidence, call control, urgency creation, objection handling, and whether a deposit was asked, producing a 0–100 score plus strengths, weaknesses, and coaching suggestions logged to the agent scorecard.',
          },
          {
            q: 'How does the agent role engine decide who gets promoted or demoted?',
            a: 'A weekly rollup feeds 4-week and 8-week revenue averages per agent into a role engine that automatically moves closers between Fresh, Junior, and Senior tiers, with each tier gaining access to progressively higher-value leads — replacing subjective, manual tier assignment with one tied directly to measured performance.',
          },
          {
            q: 'What triggers a deposit request and how is it collected?',
            a: 'Moving a lead\'s pipeline stage to "Deposit Ask" triggers the deposit chain automatically: the amount is calculated from an agent override or 10% of the quote with a $300 floor, a Clover hosted-checkout link is texted to the lead, and a Clover webhook confirms payment, moves the opportunity to Booked, and credits the closing agent\'s revenue scorecard.',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        stack: ['n8n', 'Zendesk', 'Google Vision AI', 'GPT-4o', 'ElevenLabs', 'Aircall', 'Microsoft SQL Server', 'Google Cloud Storage'],
        faqs: [
          {
            q: 'What is an AI-powered support ticket triage automation?',
            a: 'An AI-powered support ticket triage automation automatically reads a new customer support ticket — including its text, any attachments, or a linked call recording — and generates a summary, sentiment assessment, and a suggested first reply for a human agent to review, typically posting that output as an internal note rather than sending it directly to the customer.',
          },
          {
            q: 'How is OCR used to process image and PDF attachments in a support ticket workflow?',
            a: "OCR (Optical Character Recognition), such as Google Vision AI, extracts machine-readable text from image or PDF attachments a customer submits — like a screenshot of an error message or a scanned invoice — so that text can be included in the context an AI model uses to summarize the ticket and draft a response, rather than the AI being blind to what the attachment actually contains.",
          },
          {
            q: 'How can recorded phone support calls be automatically summarized?',
            a: 'A recorded call — for example, from a phone system like Aircall — can be downloaded, converted to text with a speech-to-text service such as ElevenLabs, and passed to a large language model like GPT-4o to generate a written summary and sentiment analysis, turning an audio recording into structured, searchable text without manual transcription.',
          },
          {
            q: 'Why should AI-generated ticket responses be posted as internal notes instead of sent directly to customers?',
            a: "Posting AI-drafted responses as internal, agent-only notes keeps a human reviewer in the loop before any AI-generated content reaches a customer, protecting against inaccurate, incomplete, or wrongly-toned responses that a fully automated reply system could otherwise send without review.",
          },
          {
            q: 'What is a "central intelligence hub" in a customer support automation?',
            a: "A central intelligence hub is a shared database — here, Microsoft SQL Server — that every AI-support-automation workflow writes to, typically separating raw input data, AI-generated output, and a linking record per interaction, so support data from different triggers (new tickets, ongoing conversations, call recordings) can be queried and analyzed together instead of staying siloed inside individual ticket records.",
          },
          {
            q: 'How does an automation prevent an AI agent from posting duplicate responses to the same support ticket?',
            a: "A duplication-prevention check confirms whether a matching AI-generated comment already exists on a ticket before posting a new one, and a completion tag is applied once processing finishes so the same trigger event isn't reprocessed if it fires again — for example, from a webhook retry or a re-applied tag.",
          },
          {
            q: "What's the difference between ticket-level, conversation-level, and call-level AI analysis in a support automation suite?",
            a: "Ticket-level analysis runs when a new ticket is first created, evaluating the initial message and any attachments. Conversation-level analysis runs on demand — typically triggered by an agent adding a specific tag — and analyzes the full back-and-forth history to draft a context-aware reply. Call-level analysis is triggered separately to transcribe and summarize a linked phone call recording. Each addresses a different moment in the support lifecycle, rather than one generic \"analyze this ticket\" step.",
          },
          {
            q: 'Why wait and poll for OCR job completion instead of processing the result immediately?',
            a: "OCR on documents like PDFs is often processed asynchronously by the provider, so the extracted text isn't available the instant the job is submitted — a workflow needs to wait a set interval, check the job's status, and repeat if necessary before it can safely combine and use the OCR results, rather than assuming the text is ready immediately.",
          },
        ],
      },
    ],
